<<<<<<< HEAD
**Лабораторная работа 1. Часть 2: Знакомство с Next.js** (адаптировано для Windows + VSCode)
...
**Требования:**
- Установленный [Node.js](https://nodejs.org/) (версия 18.17 или выше)
- Редактор кода (рекомендуется VSCode)
- Терминал: можно использовать встроенный терминал VSCode (PowerShell, cmd или Git Bash)

Откройте терминал в VSCode (View → Terminal) или любой командной строке Windows и выполните следующие команды:

```bash
# Создание проекта Next.js с TypeScript, Tailwind CSS и папкой app/
...
# Установка дополнительной библиотеки для работы с датами
npm install date-fns
```

**Примечание:** Если вы используете PowerShell, команды те же. Если возникает ошибка выполнения сценариев, временно разрешите выполнение: `Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass` или используйте cmd.

#### **Просмотр структуры проекта**

В Windows нет встроенной команды `tree` (хотя её можно установить). Самый простой способ – открыть папку `portfolio-site` в VSCode и посмотреть структуру в боковой панели "Explorer".  
Если вы хотите увидеть структуру в терминале, используйте PowerShell:

```powershell
# Показать содержимое папки рекурсивно (только папки и файлы .tsx/.ts)
Get-ChildItem -Recurse | Where-Object { $_.Name -match '\.tsx?$|\.json$' } | Sort-Object FullName
```

Ожидаемая структура (минимальная):
```
portfolio-site/
├── app/
│   ├── layout.tsx
...
Код файлов **не зависит от операционной системы**. Скопируйте содержимое, приведённое ниже, в соответствующие файлы вашего проекта.
...
Создайте тип для статей и массив данных:
...
Команды выполняются в терминале Windows (PowerShell/cmd/Git Bash) из папки проекта.
...
**Примечание:** Если порт 3000 занят, Next.js автоматически предложит другой (например, 3001).
...
3. Залогиньтесь и запустите деплой (в терминале):
```bash
vercel login
vercel
```

4. Следуйте инструкциям в терминале (выберите проект, подтвердите настройки)
5. После деплоя вы получите ссылку на ваше приложение (например, `portfolio-site.vercel.app`)
...
### **7. Полезные команды для Windows (PowerShell)**
...
# Поиск всех файлов .tsx и .ts в проекте
Get-ChildItem -Recurse -Include *.tsx, *.ts | Sort-Object Name

# Просмотр структуры проекта в виде дерева (если установлен модуль)
# Install-Module -Name PSTree -Force
# pstree
```

Если вы используете **Git Bash** (в составе Git for Windows), можно применять привычные команды Linux: `rm -rf .next`, `find . -name "*.tsx"` и т.д. – они будут работать.
...
2. **Проверяйте типы:** TypeScript поможет избежать многих ошибок – обращайте внимание на подсказки в VSCode.
3. **Используйте автодополнение:** VSCode отлично работает с Next.js и TypeScript.
4. **Тестируйте навигацию:** Переходите по всем ссылкам, проверяйте корректность.
5. **Читайте ошибки:** Next.js даёт подробные сообщения об ошибках прямо в браузере и терминале.
6. **Для удобства работы с терминалом в VSCode можно выбрать Git Bash (если установлен) – тогда команды будут совместимы с Linux.**
...
>>>>>>> 2d88e62823627bdde1a88fe3d5893ce5f579bc7f
